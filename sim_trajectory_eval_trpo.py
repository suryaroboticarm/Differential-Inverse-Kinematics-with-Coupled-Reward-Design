...
Usage:
    ./isaaclab.sh -p scripts/sim_trajectory_eval_trpo.py \\
        --checkpoint TRPO/model.zip --trajectory circle
"""
...
def run_eval(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    # Load policy
    print(f"Loading TRPO from: {args_cli.checkpoint}")
    model = TRPO.load(args_cli.checkpoint, device=device)
    ...
    for step in range(total_steps):
        ...
        obs = np.concatenate([
            ...
            rel_quat,       # 4
            ee_vel_np,      # 6
        ]).astype(np.float32).reshape(1, -1)  # (1, 25)
