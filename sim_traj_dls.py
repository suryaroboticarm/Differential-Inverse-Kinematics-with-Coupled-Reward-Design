...
def compute_jacobian(joint_pos, delta=1e-4):
    J = np.zeros((6, 6))
    fk0 = forward_kinematics(joint_pos)[-1]  # Get end-effector transform (T0e)
    p0 = fk0[:3, 3]
    R0 = fk0[:3, :3]
    for i in range(6):
        dq = joint_pos.copy()
        dq[i] += delta
        fk1 = forward_kinematics(dq)[-1]  # Get end-effector transform (T0e)
        J[:3, i] = (fk1[:3, 3] - p0) / delta  # linear velocity part
        # angular velocity via skew-symmetric of R1*R0^T
        dR = fk1[:3, :3] @ R0.T
        J[3, i] = dR[2, 1] / delta
        J[4, i] = dR[0, 2] / delta
        J[5, i] = dR[1, 0] / delta
...
def dls_velocity_control(joint_pos, target_pos, target_orn, ee_pos, ee_quat, lam=0.05):
    J = compute_jacobian(joint_pos)
    
    # Position error
    dp = target_pos - ee_pos
    
    # Orientation error (axis-angle from quaternion)
    rel_q = quat_mul_np(target_orn, quat_conj(ee_quat))
    angle = 2 * np.arccos(np.clip(abs(rel_q[0]), 0, 1))
    axis = rel_q[1:] / (np.sin(angle/2) + 1e-8)
    dR = axis * angle
    
    x_err = np.concatenate([dp, dR])
    
    # DLS pseudoinverse
    J_dls = J.T @ np.linalg.inv(J @ J.T + lam**2 * np.eye(6))
    
    return J_dls @ x_err  # joint velocities in rad/s

class VelocityQPController:
    def __init__(self, dq_max=1.0, lam=0.001):
        self.dq_max = dq_max
        self.lam = lam
        self.solver = osqp.OSQP()
        self.initialized = False

    def compute_jacobian(self, joint_pos, delta=1e-4):
        J = np.zeros((6, 6))
        fk0 = forward_kinematics(joint_pos)[-1]
        p0 = fk0[:3, 3]
        R0 = fk0[:3, :3]
        for i in range(6):
            dq = joint_pos.copy()
            dq[i] += delta
            fk1 = forward_kinematics(dq)[-1]
            J[:3, i] = (fk1[:3, 3] - p0) / delta
            dR = fk1[:3, :3] @ R0.T
            J[3, i] = dR[2, 1] / delta
            J[4, i] = dR[0, 2] / delta
            J[5, i] = dR[1, 0] / delta
    ...
    def compute_cartesian_error(self, ee_pos, ee_quat, target_pos, target_orn):
        dp = target_pos - ee_pos
        # quaternion error → axis-angle
        rel_q = quat_mul_np(target_orn, quat_conj(ee_quat))
        angle = 2 * np.arccos(np.clip(abs(rel_q[0]), 0.0, 1.0))
        axis = rel_q[1:] / (np.sin(angle / 2) + 1e-8)
        return np.concatenate([dp, axis * angle])

    def setup(self, J, x_err):
        """First-time OSQP problem setup."""
        n = 6  # joint DOF

        # Objective: min 0.5*dq'*P*dq + q'*dq
        # P = I + lam * J'J,  q = -J'*x_err
        P = np.eye(n) + self.lam * J.T @ J
        q = -J.T @ x_err

        # Constraints: -dq_max <= dq <= dq_max
        # written as: l <= I*dq <= u
        A = np.eye(n)
        l = -self.dq_max * np.ones(n)
        u =  self.dq_max * np.ones(n)

        P_sparse = sp.csc_matrix(P)
        A_sparse = sp.csc_matrix(A)

        self.solver.setup(
            P_sparse, q, A_sparse, l, u,
            verbose=False,
            eps_abs=1e-4,
            eps_rel=1e-4,
            max_iter=1000,
    ...
    def solve(self, joint_pos, target_pos, target_orn, ee_pos, ee_quat):
        J = self.compute_jacobian(joint_pos)
        x_err = self.compute_cartesian_error(ee_pos, ee_quat, target_pos, target_orn)

        P = np.eye(6) + self.lam * J.T @ J
        q = -J.T @ x_err

        if not self.initialized:
            self.setup(J, x_err)
            self._prev_x = np.zeros(6)
            self._prev_y = np.zeros(6)
        else:
            self.solver.update(
                Px=sp.csc_matrix(P).data,
                q=q
            )

        # warm start always after initialization check
        self.solver.warm_start(x=self._prev_x, y=self._prev_y)
        ...
        if result.info.status == 'solved':
            ...
            print(f"[WARN] OSQP status: {result.info.status}, returning zeros")
            self._prev_x = np.zeros(6)
            self._prev_y = np.zeros(6)
            return np.zeros(6)
...
def run_eval(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    for step in range(total_steps):
        ...
        # ── Policy inference ──────────────────────────────────────────────
        qp_controller = VelocityQPController(dq_max=1.0, lam=0.001)
        action = qp_controller.solve(joint_pos_np, target_pos_np, target_orn, ee_pos_np, ee_quat_np)

        # action = dls_velocity_control(joint_pos_np, target_pos_np, target_orn, ee_pos_np, ee_quat_np, lam=0.05)
        # print(f"Raw action (rad/s): {action}")
